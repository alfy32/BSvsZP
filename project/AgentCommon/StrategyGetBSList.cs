...
{
  public class StrategyGetBSList : ExecutionStrategy
  {
    private Agent agent;

    public StrategyGetBSList(int conversationId, Agent agent)
      : base(conversationId)
    {
      this.agent = agent;
    }
    ...
    private void handleResponse(Envelope envelope)
    {
      if (envelope.message.MessageTypeId() == Message.MESSAGE_CLASS_IDS.AgentListReply)
      {
        ...
        {
          StatusMonitor.get().post("Recieved BrilliantStudentList");
